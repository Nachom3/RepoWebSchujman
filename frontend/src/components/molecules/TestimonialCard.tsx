...
export function TestimonialCard({ item, index }: Readonly<TestimonialCardProps>) {
  return (
    <motion.div
      {...stagger(index)}
    >
      <Card surface="solid" padding="md" className="!rounded-2xl !p-7">
        <Stars />
        ...
        <div className="flex items-center gap-3 pt-4 border-t border-border">
          <div
            className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold ${item.avatarBg}`}
          >
            {item.initials}
          </div>
          <div>
