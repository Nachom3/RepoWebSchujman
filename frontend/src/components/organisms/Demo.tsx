        ...
        <motion.div className="max-w-4xl mx-auto" {...fadeInUp}>
            ...
            <div className="p-5 sm:p-7 grid lg:grid-cols-3 gap-5">
              ...
              <div className="lg:col-span-2 space-y-5">
                <div className="grid grid-cols-4 gap-3">
                  {demoMetrics.map((m) => (
                    <div key={m.label} className="bg-night-muted rounded-xl p-3 text-center">
                      <p className="text-xs text-night-foreground/40 mb-1">{m.label}</p>
                      <p className={`text-xl font-bold ${metricColorMap[m.fg]}`}>{m.value}</p>
                    </div>
                  ))}
                </div>
          ...
          <p className="text-center mt-4 text-sm text-muted-foreground">
            ...
            <button
              ...
              className="text-primary underline underline-offset-2 font-medium hover:text-primary-hover"
            >
              Agendá una demo
            </button>{' '}
